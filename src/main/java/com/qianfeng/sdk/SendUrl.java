 ...
 * @Description: 该类专门用于发送已经构建的好的url的http请求
 *
 * 1.定义一个Url队列--queue
 * 2.创建一个单例对象--sendUrl
 * 3.定义一个私有构造器--SendUrl
 * 4.创建共有的获取该类的实例的方法--getInstance()
 * 5.将url添加自己的发送URl的队列中---addUrlToQueue
 * 6.循环将队列中的url进行发送--sendUrl()
 * 7.用于发送url的工具类--class HttpUtil
 *
 */
